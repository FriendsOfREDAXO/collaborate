...
//                } catch(\Exception $e) {}
    ...
    /**
     * set plugin configs (from yaml) when installing it
     * set param null to re-set all enabled plugins configs
     ...
     * @created 19.09.2022
     */
    public static function setPluginConfig(?string $plugin) {
        // specific addons
        if(!is_null($plugin)) {
            $p = rex_plugin::get("collaborate", $plugin);
        ...
        // all enabled plugins
            ...
            $plugins = rex_addon::get("collaborate")->getAvailablePlugins();
        }

        foreach($plugins as $p) {
            $packageFile = rex_path::plugin("collaborate", $p->getName(), rex_package::FILE_PACKAGE);
            ...
            try {
                $yamlConfig = rex_file::getConfig($packageFile);
            } catch (rex_yaml_parse_exception $e) {
                throw new Exception(rex_i18n::msg('package_invalid_yml_file').' '.$e->getMessage());
            }

            // check scope vars
            if(isset($yamlConfig['upstream-scope'])) {
                $p->setConfig("upstream-scope", (int)$yamlConfig['upstream-scope']);
            }
            if(isset($yamlConfig['downstream-scope'])) {
                $p->setConfig("downstream-scope", (int)$yamlConfig['downstream-scope']);
            }
        }
    }
}